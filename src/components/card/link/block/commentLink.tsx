...
const CommentLink: React.FC<LinkCardCommentLinkProps> = ({ link }) => (
  <Link href={`/sharelink/${link.identifier}/${link.domain}/${link.slug}`}>
    <a className="flex items-center justify-start ml-4 group">
      <i className="text-xl font-semibold text-green-300 transition duration-300 ease-in-out delay-75 bx bx-comment-dots group-hover:text-green-500"></i>
